...
- (void)viewDidLoad {
    ...
    [self getMarvelComicEventList];
    NSURL *imageURL = [NSURL URLWithString:@"http://i.annihil.us/u/prod/marvel/i/mg/3/20/5232158de5b16/portrait_incredible.jpg"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSDictionary *item = [[NSDictionary alloc]initWithDictionary:self.eventListArray[indexPath.row]];
...
-(void)getMarvelComicEventList
{
    ...
    NSURL *marvelCharURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@%@/events?ts=%@&apikey=%@&hash=%@",MARVEL_SERVER_URL,MARVEL_GET_COMIC_DETAIL_API_URL,self.comicID,TS,MARVEL_PUBLIC_KEY,hashString]];
    ...
    [NSURLConnection sendAsynchronousRequest:[NSURLRequest requestWithURL:marvelCharURL] queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error) {
        ...
        if (error1 != nil) {
            ...
            NSLog(@"Event DATA %@ data : %@",[jsonDict objectForKeyedSubscript:@"status"],[jsonDict objectForKeyedSubscript:@"data"]);
            NSMutableArray *eventInfoArray = [NSMutableArray arrayWithArray:[[jsonDict objectForKeyedSubscript:@"data"] objectForKey:@"results"]];
           
            for (int count = 0; count < [eventInfoArray count]; count++) {
                [self.eventListArray addObject:[eventInfoArray objectAtIndex:count]];
            }
            dispatch_async(dispatch_get_main_queue(), ^{
                //NSLog(@"Array Count : %lu Array Data :%@",(unsigned long)self.eventListArray.count,self.eventListArray);
